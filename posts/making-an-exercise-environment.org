...
When exporting the file to ~html~ via my homepage I would like to see the exercises, hints and solutions printed like so:
...
After some searching on the web, I found the [[http://alhassy.com/org-special-block-extras/][~org-special-blocks-extra~]] package which proved promissing for my goal.
...
#+begin_solution
I need to install ~org-special-blocks-extra~, and this command prevents that.
#+end_solution
#+end_exercise

Since I need functionality from the special org blocks, I have to update the ~init.el~ anyway, so here it is.
...
I adapted the fontsizes and the padding of the ~details~ block of ~org-special-blocks-extra~.
I just copied the relevant part of  ~org-special-blocks-extra.el~, removed the documentation string, and changed the padding and font size.
#+begin_src emacs-lisp :tangle "init.el"
...
Let's test in on this file. As this file sits in the ~posts~ we need to include the path to  ~init.el~.
#+begin_src shell
/usr/bin/emacs --batch -l ../plugins/orgmode/init.el --eval '(nikola-html-export "making-an-exercise-environment.org" "~/tmp/output.html")'
#+end_src
