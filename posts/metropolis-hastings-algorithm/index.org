...
\begin{equation*}
p(x) \approx \frac{c(A)}{N}  \frac{1}{|A|} = \frac{c(A)}{N} \frac{2}{n},
\end{equation*}
where $n$ is the number of bins; the factor comes from the fact that the interval $[-1,1]$ is chopped up into $n$ interval.
