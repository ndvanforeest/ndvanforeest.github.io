...
\newthought{With this refresher} we can derive some useful properties of the minimum \(L=\min\{X, Y\}\), where \(Y\sim \Geo{p}\) and independent of \(X\). For this we use the fundamental bridge and 2D LOTUS, which in general read like
  \begin{equation*}
...
\newthought{Now we can} combine these facts with the properties of the maximum \(M=\max\{X, Y\}\).
...
\newthought{Here is the first} idea.
...
\newthought{And here is} the third idea.
...
\newthought{In conclusion} we verified the correctness of \(\E M = 2\E X - \E L\) in three different, and useful ways. Let us now focus on exponential rvs rather than geometric rvs.
\subsection{Continuous memoryless rvs}
\label{sec:org0bba81d}
...
\newthought{First we need} to recall some basic facts about the exponential distribution.
...
\newthought{Now we can} shift our attention to the rvs \(L\) and \(M\).
...
\newthought{We can also} compute the densities \(f_{M}(y)\) (and \(f_{L}(x)\) by marginalizing the joint density \(f_{L,M}(x,y)\).
...
\newthought{We did a} number of checks for the case \(X, Y, \iid, \sim\Exp{\lambda}\),
...
\newthought{Finally}  a third  way to check the above results, namely by verifying  \cref{eq:7}, i.e.  \(\E{M-L} = \E X\).
...
\newthought{This makes me} wonder whether \(M-L\) and \(L\) are also independent for the discrete case, i.e.,  when  \(X, Y\) iid and \(\sim \Geo{p}\).
...
\newthought{All in all}, we have checked and double checked all our expressions and limits for the geometric and exponential distribution.
We had success too: the solution of the last exercise provides the key to understand why\textasciitilde{}\eqref{eq:6} and \eqref{eq:7} are true for exponentially distributed rvs, but not for geometric random variables.
...
While \cref{eq:40} is correct, I am still not happy with the second part of\textasciitilde{}\eqref{eq:40} as I find it hard/unintuitive to interpret.
...
\newthought{I am nearly} happy, but I want to see that\textasciitilde{}\eqref{eq:44}, which is correct for discrete rvs, has also the correct limiting behavior.
\begin{exercise}
