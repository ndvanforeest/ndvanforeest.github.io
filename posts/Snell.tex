...
\author{Nicky}
\date{\today}
\title{House Selling and Buying, Snell's Envelope}
\begin{document}
...
\end{exercise}
\begin{exercise}
Conclude  from \cref{snell-eq1} that:
\begin{align*}
...
\begin{itemize}
\item \href{https://www.ihes.fr/\~bodineau/docs/MAP432.pdf}{Bodineau, Modélisation de phénomènes aléatoires}
\item \href{https://www.math.ucla.edu/\~tom/Stopping/sr2.pdf}{Ferguson lecture notes}. These notes provide a formal statement of the house selection problem and some interesting discussions on further work.
\end{itemize}
...
\subsection{A heuristic procedure}
\label{sec:org972b91f}
...
\label{sec:org02420fd}

We formalize the problem specification so that we can apply Snell's envelope to prove that the sampling policy with threshold \cref{eq:snell2}just developed is optimal.
...
\begin{enumerate}
\item \(\P{\xi_i = 1} = 1/i\) for \(i=1, \ldots n\),
\item The rvs \(\{\xi_i\}\) are independent.
\end{enumerate}
...
\begin{exercise}
Use \hyperref[th:snell]{Snell's theorem} to explain that the stopping time \cref{eq:snell2} is optimal.
\begin{solution}
