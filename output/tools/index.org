...
- =parole= to play video, which is pre-installed on manjaro. Before I used =vlc=.
...
- =pinta= for on screen drawing. However, now I write on the ipad and share the screen of the ipad via =uxplay=. (I tried to support the process with a wacom device and ~xournal~ to draw and write. But I found it very unpractical and unnatural to write on the wacom pad and see the effect on the screen. So I quit this.)
